...
from mssa_src.prediction_models.ts_meta_model import TSMM
from mssa_src.tsUtils import index_ts_mapper, index_ts_inv_mapper
from mssa_src.tsUtils import unnormalize
...
class mSSA(object):
    ...
    def predict(self, col_name, t1, t2=None, num_models=10, confidence_interval=True, use_imputed=False,
        ...
        if t2 < update_index:
            ...
            if self.uq: var = self._get_forecast_range_local(t1, t2, self.var_model, ts_no, num_models,
                                                             use_imputed=use_imputed)
    ...
    def _get_imputation_range_local(self, t1, t2, model, ts_no):
        ...
        if m1 == m2:
            ...
            if (m1 < last_model - 1 and m1 != 0):
                ...
                Result = 0.5 * unnormalize(p1.T.flatten() / model.p, norm_mean[0][ts_no],
                                           norm_std[0][ts_no]) + 0.5 * unnormalize(
            ...
            for m in range(m1, m2 + 1 + (m2 < last_model)):
                ...
                p = np.dot(U * S, V.T)
                Result[i:i + length] += 0.5 * unnormalize(p.T.flatten()[tsrow_i:tsrow_f] / model.p,
